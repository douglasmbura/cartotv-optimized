...
import { Menu, Users, FileText, Shield, Twitter, Facebook, Youtube, Globe, BookOpen } from 'lucide-react'
...
const languages = ['en','es','fr','pt','ar','de','hi','zh','sw','id','ru'] as const

function getCurrentLang(pathname: string): string {
  const parts = pathname.split('/').filter(Boolean)
  return languages.includes(parts[0] as any) ? parts[0] : 'en'
}
          ...
          <nav className="space-y-2">
            {menuItems.map(item => (
              <Link
                ...
                className="w-full flex items-center gap-3 px-4 py-3 rounded-lg hover:bg-secondary/50 transition-colors text-left text-foreground"
              >
