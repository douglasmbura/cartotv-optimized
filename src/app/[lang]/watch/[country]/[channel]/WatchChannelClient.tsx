...
interface Props {
  ...
  relatedFromManifest: ManifestChannel[]
}

export function WatchChannelClient({
  lang, countrySlug, channelSlug,
  resolvedChannelName, resolvedCountryName, relatedFromManifest
}: Props) {
  ...
  // Fetch live M3U for actual stream URLs
  useEffect(() => {
  ...
  // Find current channel in live M3U data
  ...
  // Related: prefer live M3U same-category channels, fall back to manifest
  const relatedChannels = useMemo(() => {
    if (channels.length > 0 && currentChannel) {
      return channels
        .filter(ch => ch.name !== currentChannel.name && ch.category === currentChannel.category)
        .slice(0, 12)
    }
    return []
  ...
  const handleShare = async (channel: Channel) => {
    const shareText = `Watch ${channel.name} from ${resolvedCountryName} free on CartoTV!`
    const shareUrl = window.location.href
    if (navigator.share) {
      try { await navigator.share({ title: channel.name, text: shareText, url: shareUrl }) } catch {}
    } else {
      await navigator.clipboard.writeText(`${shareText}\n${shareUrl}`)
      toast({ title: t('channels.linkCopied'), description: t('channels.linkCopiedDesc') })
    }
  }

  // Render states
  const showPlayer = !loading && currentChannel
  const showNotFound = !loading && !currentChannel && !error
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      <div className="max-w-5xl mx-auto px-4 py-6">
        ...
        {/* Channel not found in M3U — show manifest-based info page */}
        {showNotFound && (
          <div className="text-center py-16">
            ...
            <p className="text-muted-foreground mb-2">
              from {resolvedCountryName} — stream may be temporarily unavailable.
            </p>
            <div className="flex gap-3 justify-center mt-6">
              <Button onClick={() => router.push(`/${lang}/watch/${countrySlug}`)}>
                Browse {resolvedCountryName} channels
              </Button>
        ...
        {showPlayer && currentChannel && (
          <>
            {/* Player */}
            <div className="mb-6">
              <InlineVideoPlayer channel={currentChannel} />
            </div>
            ...
            <div className="flex items-start gap-4 mb-8">
              ...
              <div className="flex-1 min-w-0">
                <h1 className="text-2xl font-bold mb-1">
                  Watch {currentChannel.name} Live — Free Stream from {resolvedCountryName}
                </h1>
                <div className="flex items-center gap-3 text-sm text-muted-foreground mb-3">
                  <span className="flex items-center gap-1">
                    <Eye className="w-3.5 h-3.5" />
                    <span className="text-red-400">{formatViews(currentChannel.liveViews)} watching</span>
                  </span>
                  {currentChannel.category && (
                    <span className="px-2 py-0.5 rounded bg-primary/20 text-primary text-xs">{currentChannel.category}</span>
                  )}
                  {flag && (
                    <span className="flex items-center gap-1">
                      <img src={flag} alt={resolvedCountryName} className="w-4 h-3 object-cover rounded" />
                      {resolvedCountryName}
                    </span>
                  )}
                </div>
                <div className="flex items-center gap-2">
                  ...
                  <Button variant="outline" size="sm" onClick={() => handleShare(currentChannel)}>
                    <Share2 className="w-4 h-4 mr-1" />Share
                  </Button>
                  <Button variant="ghost" size="sm" onClick={() => router.push(`/${lang}/watch/${countrySlug}`)}>
                    <ArrowLeft className="w-4 h-4 mr-1" />All channels
                  </Button>
                </div>
              </div>
            </div>
            ...
            <section className="prose prose-sm prose-invert max-w-none mb-8">
              ...
              <p>
                {currentChannel.name} is a {currentChannel.category?.toLowerCase() || 'television'} channel
                broadcasting from {resolvedCountryName}. Watch {currentChannel.name} live online for free
                on CartoTV — no registration, no subscription, no downloads required.
                {currentChannel.category && ` As a ${currentChannel.category.toLowerCase()} channel, ${currentChannel.name} delivers quality ${currentChannel.category.toLowerCase()} programming to viewers worldwide.`}
              </p>
              <p>
                CartoTV provides free access to thousands of live TV channels from {resolvedCountryName} and
                178 other countries worldwide. Whether you&apos;re looking for news, sports, entertainment, or
                cultural programming from {resolvedCountryName}, CartoTV has you covered with reliable,
                high-quality streams available 24/7.
              </p>
            ...
            {/* Related channels from live M3U */}
            {relatedChannels.length > 0 && (
              <section className="mb-8">
                <h2 className="text-lg font-bold mb-3">
                  More {currentChannel.category || ''} channels from {resolvedCountryName}
                </h2>
                    ...
                    <Link key={`${ch.name}-${i}`} href={`/${lang}/watch/${countrySlug}/${toChannelSlug(ch.name)}`}
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <p className="text-xs text-muted-foreground flex items-center gap-1">
                          <Eye className="w-3 h-3" /><span className="text-red-400">{formatViews(ch.liveViews)}</span>
                        </p>
                      </div>
        ...
        {/* Related channels from manifest (always shown — visible even without live data) */}
        {!showPlayer && relatedFromManifest.length > 0 && (
          <section className="mb-8">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-2">
              {relatedFromManifest.map(ch => (
                <Link key={ch.slug} href={`/${lang}/watch/${countrySlug}/${ch.slug}`}
                  className="group flex items-center gap-2 p-3 rounded-lg bg-secondary/50 hover:bg-secondary transition-colors">
