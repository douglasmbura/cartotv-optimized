...
import { getCountryBySlug, toSlug } from '@/utils/countrySlug'
...
export function WatchCountryClient({ lang, countrySlug, seoContent }: Props) {
  ...
  const filteredChannels = useMemo(() => channels.filter(ch => {
    ...
    const matchCat = selectedCategory === 'All' || ch.category?.toLowerCase().includes(selectedCategory.toLowerCase())
  ...
  const handleShare = async (channel: Channel) => {
    const shareText = `Watch ${channel.name} from ${countryData?.name} free on CartoTV!`
    ...
    return (
      <div className="min-h-screen bg-background text-foreground flex items-center justify-center">
        <div className="text-center">
          ...
          <h1 className="text-xl font-bold mb-2">{t('countryPage.countryNotFound')}</h1>
          <Button onClick={() => router.push(`/${lang}/watch`)}>{t('countryPage.browseAll')}</Button>
        </div>
      </div>
    )
  }
    ...
    <div className="min-h-screen bg-background text-foreground">
      <div className="max-w-4xl mx-auto px-4 py-8">
        ...
        <div className="flex items-center gap-3 mb-6">
          <Button variant="ghost" size="icon" onClick={() => router.push(`/${lang}/watch`)}>
        ...
        <div className="flex items-center gap-1 overflow-x-auto pb-3 mb-4">
          ...
          {categories.slice(0, 8).map(cat => (
            <Button key={cat} variant={selectedCategory === cat ? 'default' : 'ghost'} size="sm"
        ...
        {!loading && !error && filteredChannels.length === 0 && (
          <p className="text-center py-12 text-muted-foreground">{t('channels.noChannels')}</p>
        )}
            ...
            <div key={`${ch.name}-${i}`} className="group flex items-center gap-3 p-3 rounded-lg bg-secondary/50 hover:bg-secondary transition-colors">
              <Link href={`/${lang}/watch/${countrySlug}/${toChannelSlug(ch.name)}`} className="flex items-center gap-3 min-w-0 flex-1">
                <div className="w-10 h-10 rounded-md bg-primary/20 flex items-center justify-center flex-shrink-0 overflow-hidden">
                  {ch.logo
                    ? <img src={ch.logo} alt={ch.name} className="w-full h-full object-cover" loading="lazy" onError={e => { (e.target as HTMLImageElement).style.display = 'none' }} />
        ...
        <section className="mt-12 prose prose-sm prose-invert max-w-none">
          {seoContent
            ...
            : <>
                <h2>{t('countryPage.watchTitle', { country: countryData.name })}</h2>
                <p>{t('countryPage.watchDescription', { country: countryData.name })}</p>
                <h3>{t('countryPage.howToWatch', { country: countryData.name })}</h3>
                <p>{t('countryPage.howToWatchText')}</p>
              </>
          }
        </section>
      </div>
